...
Function PracticeDibellanArts(Actor akActor)
		If DibellaSexCost.GetValue() > 0
			...
			EndIf
		EndIf
		If FlowerGirlsEnabled()
			Debug.Trace("Naked Dibellans: FlowerGirls method...")
			haveSexFG(akActor)
		Else
...
Function haveSexFG(Actor akActor)
	...
	AliasSpeaker.ForceRefTo(akActor)
	FlowerGirls.SetStage(40) ;random sex between akActor and player
	Player.Additem(DibellaMark,1)
	RestoreSpells()
	UpdateCurrentInstanceGlobal(DibellaSexCost)
EndFunction

Bool Function FlowerGirlsEnabled()
	Return (FlowerGirls.IsRunning() == True)
EndFunction
